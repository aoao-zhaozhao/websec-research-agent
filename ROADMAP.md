...
> 目标：Agent 能从每次扫描中学习，将成功经验沉淀为可复用技能，实现跨会话的持续进化。灵感来自 Hermes Agent 的 Skill 系统 + Curator 自维护机制。

### 核心设计

借鉴 Hermes Agent 的自进化架构：

```
扫描完成 → 自反思（scan_reflect）
    ├── 提取成功模式 → skill_create（沉淀技能）
    ├── 更新已有技能 → skill_patch（改进优化）
    └── 存入 SessionDB → 跨会话可检索
下次扫描 → skill_load → 加载相关技能增强 System Prompt
Curator（后续）→ 定期审查技能 → 合并/归档/精炼
```
...
| 模块 | 内容 |
|---|---|
| **技能系统** | `skills/` 目录 + `SkillManager`：`skill_create` / `skill_load` / `skill_list` / `skill_patch` / `skill_delete` |
| **技能格式** | SKILL.md（YAML frontmatter + Markdown body），兼容 agentskills.io 标准 |
| **技能分类** | 按漏洞类型组织：`sqli/` `xss/` `lfi/` `ssrf/` `css_injection/` `auth/` `recon/` `csp_bypass/` |
| **自反思 Prompt** | 新增 `scan_reflect` 工具 + §5 反思工作流：扫描完成后自动分析得失，生成结构化技能文档 |
| **数据库** | SQLite + SQLAlchemy，保存 scans / pages / findings / evidence（沿用 v1.3 原有计划） |
...
| **System Prompt** | 新增 §5 自进化工作流 + Agent 可主动调用 skill_create 沉淀经验 |
| **知识库动态扩展** | Agent 发现新型漏洞后可调用 `knowledge_append` 向知识库追加条目（下次 RAG 索引自动生效）|

### 技能生命周期

```
active ──(7天未使用)──> stale ──(30天)──> archived
  │                       │                  │
  └── skill_patch ────────┘                  │
  (改进后重置活跃)                             └── skill_restore ──> active
```

### 交付标准

- 关闭浏览器后扫描结果不丢失（SQLite 持久化）。
- 能导出一份包含证据链的报告。
- 能对比两次扫描差异。
- **Agent 扫描完成后能自动调用 `scan_reflect` 反思并生成经验技能**。
- **技能可在后续扫描中通过 `skill_load` 加载到上下文，增强检测能力**。
- **知识库支持运行时追加，Agent 发现的新漏洞模式可沉淀为 RAG 可检索知识**。
...
| v1.3 | 自进化 + 持久化 | 技能系统(SKILL.md)、自反思循环、SQLite持久化、报告导出、复测对比、知识库动态扩展 |
